...
def update_about_page():
    ...
    # Safely insert after Founder section by splitting on the exact known end tag of that section
    idx = html.find('<div class="cc-founder-author">Founder & Principal Architect</div>')
    if idx != -1:
        end_idx = html.find('</section>', idx) + len('</section>')
        html = html[:end_idx] + "\n" + metrics_html + html[end_idx:]
    ...
    idx2 = html.find('<section class="cc-methodology">')
    if idx2 != -1:
        end_idx2 = html.find('</section>', idx2) + len('</section>')
        html = html[:end_idx2] + "\n" + principles_html + html[end_idx2:]
    ...
    # We find the specific prefooter section by looking for its class
    idx3 = html.find('<section class="cc-prefooter-sec">')
    if idx3 != -1:
        html = html[:idx3] + "\n" + tech_html + "\n" + html[idx3:]
...
def update_js():
    ...
    old = "const elementsToReveal = document.querySelectorAll('section, h1, h2, h3, img, p');"
    new = "const elementsToReveal = document.querySelectorAll('section, h1, h2, h3, img, p, .reveal-hidden');"
    js = js.replace(old, new)
    
    old_forEach = """    elementsToReveal.forEach(el => {
        // Skip small icons or specific elements"""
    new_forEach = """    elementsToReveal.forEach(el => {
        // Skip small icons or specific elements
    ...
    js = js.replace(old_forEach, new_forEach)
